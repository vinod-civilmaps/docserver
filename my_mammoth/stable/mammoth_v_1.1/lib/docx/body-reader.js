...
var wordGroupReader = require('./wordgroup-reader');
...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        "w:p": function(element) {
           // console.log("Paragraph---------------------- ", util.inspect(element, false, null))
            return readXmlElements(element.children)
                .map(function(children) {
                   
                    var properties = _.find(children, isParagraphProperties);
                    //console.log("--------------Chidren---------------------- ", util.inspect(children, false, null), util.inspect(properties, false, null));
        ...
        "w:pPr": function(element) {
            //console.log("ParagraphStyle ", util.inspect(element, false, null))
        ...
        "w:r": function(element) {
            //console.log("Run---------------------- ", util.inspect(element, true, null))
            return readXmlElements(element.children)
                .map(function(children) {
        ...
        "mc:AlternateContent": function(element) {
            var childEle =  readChildElements(element.first("mc:Choice"));

            if (childEle.length == 0) {
                childEle = readChildElements(element.first("mc:Fallback"));
            }
        ...
        "v:rect": function(element) {
            var hrAttrKey =  'o:hr';
            if (element.attributes.hasOwnProperty(hrAttrKey) && element.attributes[hrAttrKey] == 't') {
                return elementResult(new documents.Hr({color: element.attributes.fillcolor})); 
            }
            return readChildElements(element).toExtra();
        },
    };
    ...
    function readDrawingElement(element) {
        ...
        if (graphicData.length != 0) {
            var uriParts = graphicData[0].attributes.uri.split("/");
            var graphicDataType = uriParts[uriParts.length-1];

            if (graphicDataType == "picture") {
                return readPicElement(graphicData.getElementsByTagName("pic:pic")[0])
            }

            if (graphicDataType == "wordprocessingGroup") {
                var wrodGrop = wordGroupReader.createWordGroupReader(createBodyReader(options));
                return wrodGrop(graphicData[0]);
                // var blips = graphicData
                //             .firstFind("w:p")
                // return readXmlElements(blips);
            }
        }
    ...
    function readPicElement(element) {
        var blips = element
    ...
    function readBlip(element, blip) {
        var properties = element.firstFind("pic:cNvPr")[0];
        properties = properties.attributes;

        var sizeProps = element.firstFind("a:ext")[0];
        sizeProps = sizeProps.attributes;

        var altText = isBlank(properties.descr) ? properties.title : properties.descr;
        var imgSize = {
            width : isBlank(sizeProps.cx)? "100%": Math.ceil(sizeProps.cx/9525).toString() + "px",
            height: isBlank(sizeProps.cy)? "100%": Math.ceil(sizeProps.cy/9525).toString() + "px"
        }
        return readImage(findBlipImageFile(blip), altText, imgSize);
    }
    ...
    function readImage(imageFile, altText, imgSize) {
